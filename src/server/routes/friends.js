...
router.post("/request", cookieCheck, async (req, res) => {
  ...
  try {
    if (username) {
      ...
      if (frExists) {
        res.status(200).send({
          message: `Friend request for ${recipient.username} is already pending`,
        });
      } else {
        ...
        if (sentPendingFR) {
          sentPendingFR.save();
          res.status(200).send({
            message: `Your friend request was sent to ${recipient.username}`,
          });
        } else
          res.status(404).send({
            message: `Unable to send friend request to ${recipient.username}`,
          });
      }
    } else {
...
router.delete("/remove", cookieCheck, async (req, res) => {
  ...
  try {
    if (user && friend) {
      ...
      const removeFriendFriendListIndex = friend.friends
        .map((friend) => friend.id)
        .indexOf(user.id);
      ...
      if (
        removeUserFriendListIndex !== -1 &&
        removeFriendFriendListIndex !== -1
      ) {
        ...
        ChatMessage.deleteMany({
          $and: [
            { $or: [{ sender: user_id }, { sender: friend._id }] },
            { $or: [{ recipient: friend._id }, { recipient: user_id }] },
          ],
        });
        //conversation between these 2 users is deleted and
        res
          .status(200)
          .send({ message: "Removed from friendlist", removedId: friend.id });
      } else {
        res.status(404).send({
          message: "Friendship already removed or incorrect id was provided",
        });
      }
    } else {
