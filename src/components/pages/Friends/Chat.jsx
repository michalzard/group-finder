...
function Chat({ socket, setFriendListOpen }) {
  ...
  const { list, currentFriend, presence } = useSelector(
    (state) => state.friends
  );
      ...
      <section className="chat-header">
        <section className="user-info">
          ...
          <div
            ...
            style={{
              backgroundColor: isOnline(id) ? "greenyellow" : "gray",
              margin: "0 5px",
            }}
          />
        </section>
        ...
        <section className="chat-messages">
          <section className="initial-message">
            <Typography variant="caption">
              {new Date(currentFriend.createdAt).toUTCString()}
            </Typography>
            <Typography>
              {" "}
              <PersonAddIcon /> You're now chatting with{" "}
              {currentFriend.username}
            </Typography>
          </section>
          ...
          {chatMessages
            .filter(
              (message) =>
                message.to === currentFriend.id ||
                message.to === user.id ||
                (message.sender !== null &&
                  message.sender.id === currentFriend.id)
            )
            .map((message) => (
              <ChatBubble
                key={message.msg_id}
                timestamp={message.timestamp}
                sentBy={{ id: message.sender ? message.sender.id : user.id }}
...
function ChatBubble({ sentBy, text, timestamp }) {
  ...
  return (
    <div
      ...
      style={{
        justifyContent: user.id === sentBy.id ? "flex-end" : "flex-start",
      }}
    >
      ...
      <div
        className={`chat-message-bubble ${
          user.id === sentBy.id ? null : "gray-bubble"
        }`}
      ...
      >
        <section className="chat-text">
          <Typography>
            {text
              ? text
              : "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur vitae pulvinar enim, vel tristique ipsum. Ut vitae quam nibh. Integer rutrum nunc eget sodales lacinia. Nullam sit amet interdum lacus. Quisque quis eros ut nulla commodo commodo eu congue turpis. Nam lacinia sit amet risus at consectetur."}
          </Typography>
        </section>
        <section className="chat-timestamp">
          <Typography variant="caption">
            {timestamp
              ? new Date(timestamp).toUTCString().substring(0, 25)
              : new Date("2022-10-18").toUTCString().substring(0, 17)}
          </Typography>
        </section>
      </div>
...
function ColorMenu({ anchor, handleClose }) {
  const colors = Object.entries(ChatThemeColors).map((a) => {
      ...
      <div className="color-picker">
        {colors.map((color, i) => (
          <div
            key={i}
            className={`dot ${
              chatBubbleColor === color.value ? "dot-selected" : null
            }`}
            onClick={() => {
              dispatch(selectChatBubbleColor(color.value));
            }}
